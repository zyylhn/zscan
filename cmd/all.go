	...
	if conn != nil {
		...
		case 3306:
			if !notburp {
				...
				startburp := NewBurp(Password, user, Userdict, Passdict, ip, mysql_auth, 10)
				relust := startburp.Run()
				if relust != "" {
					interested_result.Store(addr, relust)
					return ip, port, nil, []string{relust}
		...
		case 6379:
			if !notburp {
				...
				if f {
					Output(fmt.Sprintf("\r[+]%v burp success:%v No authentication\n", "redis", ip), LightGreen)
					interested_result.Store(addr, "redis no authentication")
					return ip, port, nil, []string{"No authentication"}
				}
				startburp := NewBurp(Password, "", "", Passdict, ip, redis_auth, 50)
				relust := startburp.Run()
				if relust != "" {
					interested_result.Store(addr, relust)
					return ip, port, nil, []string{relust}
		...
		case 1433:
			if !notburp {
				...
				startburp := NewBurp(Password, user, Userdict, Passdict, ip, mssql_auth, 50)
				relust := startburp.Run()
				if relust != "" {
					interested_result.Store(addr, relust)
					return ip, port, nil, []string{relust}
		...
		case 21:
			if !notburp {
				...
				startburp := NewBurp(Password, user, Userdict, Passdict, ip, ftp_auth, 50)
				relust := startburp.Run()
				if relust != "" {
					interested_result.Store(addr, relust)
					return ip, port, nil, []string{relust}
		...
		case 27017:
			if !notburp {
				...
				startburp := NewBurp(Password, user, Userdict, Passdict, ip, mongodb_auth, 50)
				relust := startburp.Run()
				if relust != "" {
					interested_result.Store(addr, relust)
					return ip, port, nil, []string{relust}
		...
		case 445:
			...
			if smbRes != nil {
				Output(fmt.Sprintf("\r[+]Smbinfo:%v -> %v\n", ip, smbRes), LightGreen)
			}
			...
			if !notburp {
				...
				if f {
					Output("\r"+ip+": SMB unauthorized(printer)\n", LightGreen)
					smbRes = append(smbRes, "SMB unauthorized")
				} else {
					startburp := NewBurp(Password, user, Userdict, Passdict, ip, smb_auth, 50)
					relust := startburp.Run()
					if relust != "" {
		...
		case 135:
			...
			if oxidres != nil {
				Output(fmt.Sprintf("\r[+]Oxidinfo:%v -> %v\n", ip, oxidres), LightGreen)
			}
			return ip, port, nil, oxidres
		case 139:
			nbname, _ := netBIOS(ip)
			if nbname.msg != "" {
				Output(fmt.Sprintf("\r[+]NetBiosinfo:%v\n\t%v\n", ip, nbname.msg), LightGreen)
...
func init() {
	...
	allCmd.Flags().StringVarP(&ps_port, "port", "p", config.Default_port, "Set `port` eg:1-1000,3306,3389 or use \" appname all -p l/all\" ) to scan less port（thirty port）")
